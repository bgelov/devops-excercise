...
nginx разворачивается nginx и добавляются ssl сертификаты. По условиям, именно на Nginx просходит терминация SSL и запросы к сервисам отправляются по http. Для работы сервисов написан конфиг nginx (в services/nginx).

haproxy разворачивается в режиме tcp, на 4 уровне, т.к. по условиям терминацию SSL должен выполнять Nginx.

После выполнения деплоя сервисы будут доступны по адресам /nodejs-service и /php-service.


Процесс можно улучшить, например, добавив стадии тестирования, разворачивания на предрелизные сервера, добавив больше базовых настроек и настроек безопасности, сделать конфигурационные файлы шаблонами и т.д.
...
Для тестирования разворачивания ansible инфраструктуры в локальной среде, необходимо скопировать репозиторий на машину с установленным Ansible.

Добавить данные машины и задать переменные в файле ansible/local_test_vars.yml

Для запуска плейбука первоначальной настройки необходимо выполнить команду:

```
ansible-playbook -i local_test_inventory.yml server-setup.yml -e @local_test_vars.yml
```
